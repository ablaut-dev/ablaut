...
use ablaut::est::{Error, ImpersonalSlot, NonfiniteSlot, Number, Person, Verb};
...
fn load_adjudications() -> HashSet<(String, String)> {
    fs::read_to_string("docs/est/adjudications.tsv")
        .expect("docs/est/adjudications.tsv")
        .lines()
...
fn report(paths: &[String], s: &Scores, dropped: usize) {
    ...
    println!("== ablaut::est vs gold: {} ==", paths.join(" ∩ "));
    println!(
    ...
    fs::write("target/golden_est_mismatches.tsv", &s.mismatches).unwrap();
    println!(
        "\n{} mismatching forms written to target/golden_est_mismatches.tsv",
...
fn main() {
    ...
    if paths.is_empty() {
        paths.push("data/est/vabamorf.tsv".to_string());
        if fs::metadata("data/est/kaikki.tsv").is_ok() {
            paths.push("data/est/kaikki.tsv".to_string());
        }
    }
    let mut golds: Vec<Gold> = paths
        ...
        .map(|p| {
            let data = fs::read_to_string(p).unwrap_or_else(|e| {
                panic!("cannot read {p}: {e}. Run the scripts/est/fetch_*.sh scripts first")
            });
            parse_gold(&data)
        })
        .collect();
