...
use ablaut::spa::{Error, Number, Person, SimpleTense, Verb};
use std::collections::{BTreeMap, HashMap, HashSet};
use std::fmt::Write as _;
use std::fs;

/// CI regression gates (percent), measured against FreeLing alone
/// (99.98 / 99.96 at PR time; the margin absorbs FreeLing updates, not
...
fn load_adjudications() -> HashSet<(String, String)> {
    fs::read_to_string("docs/spa/adjudications.tsv")
        .expect("docs/spa/adjudications.tsv")
        .lines()
...
fn report(paths: &[String], s: &Scores, dropped: usize) {
    ...
    println!("== ablaut::spa vs gold: {} ==", paths.join(" ∩ "));
    println!(
    ...
    fs::write("target/golden_spa_mismatches.tsv", &s.mismatches).unwrap();
    println!(
        "\n{} mismatching forms written to target/golden_spa_mismatches.tsv",
...
fn main() {
    ...
    if paths.is_empty() {
        paths.push("data/spa/freeling.tsv".to_string());
        if fs::metadata("data/spa/kaikki.tsv").is_ok() {
            paths.push("data/spa/kaikki.tsv".to_string());
        }
    }
    let mut golds: Vec<Gold> = paths
        ...
        .map(|p| {
            let data = fs::read_to_string(p).unwrap_or_else(|e| {
                panic!("cannot read {p}: {e}. Run the scripts/spa/fetch_*.sh scripts first")
            });
            parse_gold(&data)
        })
        .collect();
