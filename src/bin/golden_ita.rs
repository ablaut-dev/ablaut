...
use ablaut::ita::{Error, Number, Person, SimpleTense, Verb};
use std::collections::{BTreeMap, HashMap, HashSet};
use std::fmt::Write as _;
use std::fs;

/// CI regression gates (percent), measured against Morph-It alone
/// (99.57 / 99.98 at PR time; the margin absorbs Morph-It updates, not
...
fn load_adjudications() -> HashSet<(String, String)> {
    fs::read_to_string("docs/ita/adjudications.tsv")
        .expect("docs/ita/adjudications.tsv")
        .lines()
...
fn report(paths: &[String], s: &Scores, dropped: usize) {
    ...
    println!("== ablaut::ita vs gold: {} ==", paths.join(" ∩ "));
    println!(
    ...
    fs::write("target/golden_ita_mismatches.tsv", &s.mismatches).unwrap();
    println!(
        "\n{} mismatching forms written to target/golden_ita_mismatches.tsv",
...
fn main() {
    ...
    if paths.is_empty() {
        paths.push("data/ita/morphit.tsv".to_string());
        if fs::metadata("data/ita/kaikki.tsv").is_ok() {
            paths.push("data/ita/kaikki.tsv".to_string());
        }
    }
    let mut golds: Vec<Gold> = paths
        ...
        .map(|p| {
            let data = fs::read_to_string(p).unwrap_or_else(|e| {
                panic!("cannot read {p}: {e}. Run the scripts/ita/fetch_*.sh scripts first")
            });
            parse_gold(&data)
        })
        .collect();
