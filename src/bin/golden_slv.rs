...
use ablaut::slv::{Error, Gender, Number, Person, Verb};
...
fn load_adjudications() -> HashSet<(String, String)> {
    fs::read_to_string("docs/slv/adjudications.tsv")
        .expect("docs/slv/adjudications.tsv")
        .lines()
...
fn report(paths: &[String], s: &Scores, dropped: usize) {
    ...
    println!("== ablaut::slv vs gold: {} ==", paths.join(" ∩ "));
    println!(
    ...
    fs::write("target/golden_slv_mismatches.tsv", &s.mismatches).unwrap();
    println!(
        "\n{} mismatching forms written to target/golden_slv_mismatches.tsv",
...
fn main() {
    ...
    if paths.is_empty() {
        paths.push("data/slv/sloleks.tsv".to_string());
        if fs::metadata("data/slv/kaikki.tsv").is_ok() {
            paths.push("data/slv/kaikki.tsv".to_string());
        }
    }
    let mut golds: Vec<Gold> = paths
        ...
        .map(|p| {
            let data = fs::read_to_string(p).unwrap_or_else(|e| {
                panic!("cannot read {p}: {e}. Run the scripts/slv/fetch_*.sh scripts first")
            });
            parse_gold(&data)
        })
        .collect();
