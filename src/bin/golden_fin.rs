...
use ablaut::fin::{Error, Number, Person, Tense, Verb};
use std::collections::{BTreeMap, HashMap, HashSet};
use std::fmt::Write as _;
use std::fs;

/// CI regression gates (percent), measured against dexonline alone.
const MIN_FORM_PCT: f64 = 98.5;
...
fn load_adjudications() -> HashSet<(String, String)> {
    fs::read_to_string("docs/fin/adjudications.tsv")
        .expect("docs/fin/adjudications.tsv")
        .lines()
...
fn report(paths: &[String], s: &Scores, dropped: usize) {
    ...
    println!("== ablaut::fin vs gold: {} ==", paths.join(" ∩ "));
    println!(
    ...
    fs::write("target/golden_fin_mismatches.tsv", &s.mismatches).unwrap();
    println!(
        "\n{} mismatching forms written to target/golden_fin_mismatches.tsv",
...
fn main() {
    ...
    if paths.is_empty() {
        paths.push("data/fin/omorfi.tsv".to_string());
        if fs::metadata("data/fin/kaikki.tsv").is_ok() {
            paths.push("data/fin/kaikki.tsv".to_string());
        }
    }
    let mut golds: Vec<Gold> = paths
        ...
        .map(|p| {
            let data = fs::read_to_string(p).unwrap_or_else(|e| {
                panic!("cannot read {p}: {e}. Run the scripts/fin/fetch_*.sh scripts first")
            });
            parse_gold(&data)
        })
        .collect();
